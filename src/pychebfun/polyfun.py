...
from typing import Callable, NewType, Optional, overload
...
class Polyfun:
    ...
    @classmethod
    def from_coeff(
        cls, chebcoeff: npt.ArrayLike, domain: Optional[Domain] = None, prune: bool = True, vscale: float = 1.0
    ):
        """
    ...
    def dichotomy(
        ...
        kmin: int = 2,
        kmax: int = 12,
        raise_no_convergence: bool = True,
    ) -> np.ndarray:
        """
    ...
    @classmethod
    def from_function(cls, f: Callable, domain: Optional[Domain] = None, N: Optional[int] = None) -> Self:
        """
    ...
    def _cutoff(cls, coeffs: np.ndarray, vscale: float) -> int:
        ...
        N = inds[0][-1] if len(inds[0]) else 0
        return N + 1

    def __init__(self, values: npt.ArrayLike = 0.0, domain: Optional[Domain] = None, vscale: Optional[float] = None):
        """
    ...
    def __add__(self, other: Self) -> Self:
        ...
        return self.from_coeff(chebsum, domain=self.domain, vscale=new_vscale)
